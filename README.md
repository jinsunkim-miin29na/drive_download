...
한 번에 전부 실행하지 말고 `시작 번호`와 `묶음 크기`를 사용하세요.
...
저장 후 성공 개수에 `50`을 입력하면 웹페이지가 `50번까지 받음`으로 기록하고 다음 시작 번호를 추천합니다. 중간에 멈추면 마지막으로 받은 번호 다음부터 다시 시작하면 됩니다.

단축어 실행 중 진행 상황은 웹페이지가 실시간으로 읽을 수 없습니다. 단축어 안에서 사진을 저장할 때마다 `Saved Count`를 1씩 올리고 `알림 보기`로 `저장 중: Saved Count / Expected Count`를 표시하세요.

이미 완료 처리된 파일은 다음 묶음에서 자동으로 제외됩니다. 같은 Drive 파일 ID는 건너뛰고, 같은 파일명/크기/MIME type 조합도 중복 후보로 보고 제외합니다.
